...
listeCycles* baseDeCyclesMinimale(grapheMol g) {
    ...
    int ** matriceAretes = matriceAretesDansCycles(g, cycles, &nb_aretes);
    printGrapheMol(g);
    printMatrice(matriceAretes, cycles->nb_cycles, nb_aretes);
...
int** matriceAretesDansCycles(grapheMol g, listeCycles *cycles, int *nb_aretes) {
    ...
    *nb_aretes = 0;
    int **matriceAretes = calloc(cycles->nb_cycles, sizeof(int*));
    ...
    for (i = 0; i < cycles->nb_cycles; i++) {
        matriceAretes[i] = calloc(*nb_aretes, sizeof(int)); 
    }
    ...
    for (i = 0; i < cycles->nb_cycles; i++) {
        ...
        if (id1 < id2) {
            arete_id = g.adjacence[id2][id1];
        }
        else {
            arete_id = g.adjacence[id1][id2];
        }
        matriceAretes[i][arete_id] = 1;
